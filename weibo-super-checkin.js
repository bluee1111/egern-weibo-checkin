...
function translateHttpError(error) {
  ...
  if (/JSON Parse|Unexpected identifier|Unexpected token|SyntaxError/i.test(raw)) {
    return new Error("接口返回了非 JSON 内容，可能是风控页或需要重新登录");
  }
...
async function requestJson(ctx, url, options = {}) {
  ...
  // ctx.http 内部会自行解析 JSON，非 JSON 内容会抛解析错误，必须整段包住
  let response;
  try {
    response =
      method === "POST"
  ...
  // 解析 body：兼容“已解析对象 / Response-like / 字符串”三种形态
  try {
...
async function captureCookie(ctx) {
  ...
  console.log(`微博 Cookie 捕获：账号 ${accountId} ${changed ? "已更新" : "无变化"}`);
  if (shouldNotify) {
    ctx.notify({
      title: "微博超话签到",
      subtitle: `账号 ${accountId}`,
      body: changed ? "Cookie 已更新，自动签到运行中" : "Cookie 获取成功，已开启自动签到",
      sound: true,
      duration: 5,
    });
  }
...
async function runCheckin(ctx) {
  ...
  if (!entries.length) {
    const result = {
      ...
      message: "没有 Cookie，请打开 m.weibo.cn 登录后再打开一次",
    };
