// 微博超话自动签到（网页获取 Cookie 版）
// 打开 m.weibo.cn 并登录后自动捕获 Cookie，每天定时签到全部关注超话。
...
function signEnabled(ctx) {
  const value = ctx.env?.ENABLE_SIGN ?? ctx.args?.ENABLE_SIGN ?? "true";
  return String(value).toLowerCase() !== "false";
}
...
async function requestJson(ctx, url, options = {}) {
  ...
  try {
    ...
    if (/JSON Parse|Unexpected identifier|Unexpected token/i.test(message)) {
      throw new Error("微博接口返回了非 JSON 内容（可能是风控页或 Cookie 失效），请重新打开 m.weibo.cn 刷新");
    }
...
async function captureCookie(ctx) {
  ...
  if (!cookie || !/(?:^|;\s*)SUB=/i.test(cookie)) return;
  ...
  if (shouldNotify) {
    await notify(ctx, "✅ 微博 Cookie 获取成功", "自动签到已就绪");
  }
}
...
async function checkinAccount(ctx, accountId, cookie) {
  ...
  if (!state.login) {
    ...
    if (!expired[accountId]) {
      ...
      await notify(ctx, "⚠️ 微博 Cookie 失效", "请重新打开 m.weibo.cn 登录并刷新页面");
    }
    throw new Error("Cookie 已失效，请重新打开 m.weibo.cn 登录");
  }
...
async function runCheckin(ctx) {
  ...
  if (!entries.length) {
    const result = { day: currentDay(), time: new Date().toISOString(), accounts: 0, total: 0, success: 0, already: 0, failed: 1, message: "没有 Cookie，请打开 m.weibo.cn 登录后刷新" };
